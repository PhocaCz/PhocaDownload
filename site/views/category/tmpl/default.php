			...
			if (!empty($this->subcategories)) {
				foreach ($this->subcategories as $valueSubCat) {
					...
					if(empty($subcategoriesLayout) || ($this->t['display_specific_layout'] == 0)){
						$subcategoriesLayout = '<div class="pd-subcategory">
						<a href="{pdlink}">{pdtitle}</a>
						<small>{pdnumdocs}</small>
						</div>';
					}
					$pdlink = JRoute::_(PhocaDownloadRoute::getCategoryRoute($valueSubCat->id, $valueSubCat->alias));
					$pdtitle = $valueSubCat->title;
					$pdnumdocs = $valueSubCat->numdoc;
					/*'<div class="pd-subcategory">
					<a href="{pdlink}">{pdtitle}</a>
					<small>{pdnumdocs}</small>
					</div>';'
					*/
					$subcategoriesLayoutParams 	= PhocaDownloadSettings::getLayoutParams('subcategory');

					$replace	= array($pdlink, $pdtitle, $pdnumdocs);
					echo str_replace($subcategoriesLayoutParams['search'], $replace, $subcategoriesLayout);
				}
